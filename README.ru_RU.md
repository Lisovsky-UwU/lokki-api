...
- Перетаскивание для сортировки и переноса между папками и коллекциями; сами коллекции переставляются так же
...
**Импорт**
- OpenAPI 3.x и Swagger 2.0, JSON или YAML, из файла или по ссылке
- Операции становятся запросами и раскладываются по папкам согласно тегам; каждый сервер из спецификации становится отдельным окружением, первое включается сразу
- Переносятся параметры, заголовки, пример тела, собранный по схеме, и схема авторизации; всё, что воспроизвести не удалось, перечисляется по окончании импорта
- Импорт всегда создаёт новую коллекцию, так что уже существующие ничего не перезапишет
...
- При запуске открывается последний workspace или приветственный экран - выбирается в «Настройки → Интерфейс»
- На приветственном экране - список последних 10 пространств с именем и путём; каждое можно убрать из списка или показать в проводнике
...
Workspace — это обычная папка, которую вы выбираете при первом запуске. Новый
workspace создаётся только в пустой папке: он занимает её целиком, поэтому
папку с уже существующими файлами приложение не примет. На открытие
существующего workspace это ограничение не распространяется.
...
cargo run --example import_spec -- <спека>  # показать, что получится из импорта OpenAPI
```
...
| `import/` | Чтение чужого формата (пока OpenAPI) в план, который затем записывает `store` |
